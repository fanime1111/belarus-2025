...
function updateAuthUI() {
    ...
    if (isAdminUser()) {
        ...
        if (navAuthButtons) {
            navAuthButtons.innerHTML = `<button id="nav-logout-btn">Выйти (админ)</button>`;
        ...
        if (navAuthButtons) {
            navAuthButtons.innerHTML = `<button id="nav-logout-btn">Выйти (${user})</button>`;
            document.getElementById('nav-logout-btn').onclick = logoutUser;
        }
    } else {
        if (authButtons) {
        ...
        if (navAuthButtons) {
            navAuthButtons.innerHTML = `<button id="nav-login-btn">Вход</button> <button id="nav-register-btn">Регистрация</button>`;
            const navLoginBtn = document.getElementById('nav-login-btn');
            const navRegisterBtn = document.getElementById('nav-register-btn');
            if (navLoginBtn) navLoginBtn.onclick = () => showModal('login-modal');
            if (navRegisterBtn) navRegisterBtn.onclick = () => showModal('register-modal');
        }
    }
}
...
function updateAuthUIWithRedirect() {
    ...
    if (isAdminUser()) {
        ...
        if (navAuthButtons) {
            ...
            if (navLogoutBtn) navLogoutBtn.onclick = logoutUser;
        }
    } else if (user) {
        if (authButtons) {
            authButtons.innerHTML = `<span class="auth-hello">Привет, <b>${user}</b>!</span> <button id="logout-btn">Выйти</button>`;
        ...
        if (navAuthButtons) {
            ...
            const navLoginBtn = document.getElementById('nav-login-btn');
            const navRegisterBtn = document.getElementById('nav-register-btn');
            if (navLoginBtn) navLoginBtn.onclick = () => redirectToMainForAuth('login');
            if (navRegisterBtn) navRegisterBtn.onclick = () => redirectToMainForAuth('register');
        }
    }
}
